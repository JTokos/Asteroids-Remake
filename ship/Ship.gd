...
func _ready():
	...
	creepy2 = AudioStreamPlayer.new()
	self.add_child(creepy2)
	creepy2.stream = load("res://sounds/creepy2.wav")

	creepy3 = AudioStreamPlayer.new()
	self.add_child(creepy3)
	creepy3.stream = load("res://sounds/creepy3.wav")

	creepy4 = AudioStreamPlayer.new()
	self.add_child(creepy4)
	creepy4.stream = load("res://sounds/creepy4.wav")
	pass

func _process(delta):

	if (disableMissile and OS.get_ticks_msec() > powerupGrabTime + powerupDisableTime):
		disableMissile = false
	if (canonFireRateUp and OS.get_ticks_msec() > powerupGrabTime + powerupDisableTime2):
		recharge_time = .35
		canonFireRateUp = false
	if (missileDelay and OS.get_ticks_msec() > powerupGrabTime + powerupDisableTime):
		missileDelay = false
	if (extraScore and OS.get_ticks_msec() > powerupGrabTime + powerupDisableTime):
		extraScore = false
		scoreMultiplier = 1
		scoreDown.play()
	if (extraSpin and OS.get_ticks_msec() > powerupGrabTime + powerupDisableTime2):
		extraSpin = false
		spin_thrust = 4
	...
	if (spawnMissileCircle and OS.get_ticks_msec() > powerupGrabTime + (1000*recharge_time*numCircles)):
		fireSoundLoud.play()
	...
	if (not playingCreepySound && randi()%5000 == 0):
		...
		if (not creepy1.playing and not creepy2.playing and not creepy3.playing and not creepy4.playing):
	...
	for i in range(contacts):
		...
		if ("Asteroid" in str(contact.get_name())): #collided with an asteroid
			...
			if ("PowerupNoMissile" in str(contact.get_name())): #disable the cannon
				disableMissile = true
				gunJam.play()
				powerupGrabTime = OS.get_ticks_msec()
